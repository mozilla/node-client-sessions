...
function encode(opts, content, createdAt){
//console.log('opts start', createdAt, opts);
    // format will be:
    ...
    if (!opts.duration) {
      opts['duration'] = 24*60*60*1000;
    }

    createdAt = createdAt || new Date().getTime();
//console.log('opts end', createdAt, opts);
    ...
    var ciphertext = cipher.update(JSON.stringify(content));
    ...
    hmacAlg.update(".");
    hmacAlg.update(opts.duration.toString());
    var hmac = hmacAlg.digest();

    return base64urlencode(iv) + "." + base64urlencode(ciphertext) + "." + createdAt + "." + opts.duration + "." + base64urlencode(hmac);
}
...
//    var hmacAlg = crypto.createHmac('sha256', this.opts.signatureKey);
  var signatureKey = deriveKey(opts.secret, 'cookiesession-signature');
  var hmacAlg = crypto.createHmac('sha256', signatureKey);
    hmacAlg.update(iv);
    hmacAlg.update(".");
    hmacAlg.update(ciphertext);
    hmacAlg.update(".");
...
//    var cipher = crypto.createDecipheriv('aes256', this.opts.encryptionKey, iv);
    var encryptionKey = deriveKey(opts.secret, 'cookiesession-encryption');
    var cipher = crypto.createDecipheriv('aes256', encryptionKey, iv);
...
Session.prototype = {
  ...
  box: function() {
    return encode(this.opts, this.content, this.createdAt);
